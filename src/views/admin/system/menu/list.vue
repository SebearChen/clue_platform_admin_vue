<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="权限名" align="left">
        <template slot-scope="{row}">
          <span class="link-type">{{ row.name ||  '-' }}</span>
        </template>
      </el-table-column>

      <el-table-column label="权限控制key" align="center">
        <template slot-scope="{row}">
          <span>{{ row.permitKey ||  '-' }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column :label="$t('table.actions')" align="center" width="100" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <router-link :to="'/system/menu/detail/'+row.id">
...
<script>
import { getList, del } from '@/api/system/menu'
...
export default {
  ...
  methods: {
    ...
    setMenus(menus, data, index) {
      for (let i in data) {
        let m = { ...data[i] }
        delete m['children']
        m['name'] = this.getTree(index) + m['name']
        menus.push(m)
        if (data[i].children) {
          this.setMenus(menus, data[i].children, index + 1)
        }
      }
    },
    ...
    getList() {
      ...
      getList(this.listQuery).then(response => {
        ...
        if (response.data && response.data.length > 0) {
          this.setMenus(menus, response.data, 0)
        }
        this.list = menus
